...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $   ))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d~ d~ > e+ ve e+ e- u~ d~
      PD(IPROC)=DX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s~ > e+ ve e+ e- c~ s~
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d~ d~ > mu+ vm mu+ mu- u~ d~
      PD(IPROC)=DX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s~ > mu+ vm mu+ mu- c~ s~
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG39 = PD(0)
...
C     
      ...
      INTEGER FUNCTION GET_NHEL39(HEL, IPART)
...
C      particle 
      ...
      GET_NHEL39 = NHEL(IPART, IABS(HEL))
      RETURN
      END
