...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $   ))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ d~ > e- ve~ e+ e- d~ d~
      PD(IPROC)=UX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ s~ > e- ve~ e+ e- s~ s~
      PD(IPROC)=CX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u~ d~ > mu- vm~ mu+ mu- d~ d~
      PD(IPROC)=UX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ s~ > mu- vm~ mu+ mu- s~ s~
      PD(IPROC)=CX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG41 = PD(0)
...
C     
      ...
      INTEGER FUNCTION GET_NHEL41(HEL, IPART)
...
C      particle 
      ...
      GET_NHEL41 = NHEL(IPART, IABS(HEL))
      RETURN
      END
