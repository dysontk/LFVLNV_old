...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $   ))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ u~ > e- ve~ mu+ mu- u~ d~
      PD(IPROC)=UX1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ c~ > e- ve~ mu+ mu- c~ s~
      PD(IPROC)=CX1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u~ u~ > mu- vm~ e+ e- u~ d~
      PD(IPROC)=UX1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ c~ > mu- vm~ e+ e- c~ s~
      PD(IPROC)=CX1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG36 = PD(0)
...
C     
      ...
      INTEGER FUNCTION GET_NHEL36(HEL, IPART)
...
C      particle 
      ...
      GET_NHEL36 = NHEL(IPART, IABS(HEL))
      RETURN
      END
