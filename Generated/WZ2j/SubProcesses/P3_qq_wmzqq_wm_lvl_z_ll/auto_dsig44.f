...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $   ))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ s~ > e- ve~ mu+ mu- d~ s~
      PD(IPROC)=UX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ d~ > e- ve~ mu+ mu- s~ d~
      PD(IPROC)=CX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u~ s~ > mu- vm~ e+ e- d~ s~
      PD(IPROC)=UX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ d~ > mu- vm~ e+ e- s~ d~
      PD(IPROC)=CX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG44 = PD(0)
...
C     
      ...
      INTEGER FUNCTION GET_NHEL44(HEL, IPART)
...
C      particle 
      ...
      GET_NHEL44 = NHEL(IPART, IABS(HEL))
      RETURN
      END
